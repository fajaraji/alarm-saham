...
5. **Penyensor saran hanya menggigit bila ada kata terlarang.** Anjuran tanpa kata beli/jual lolos utuh ke kotak masuk & Telegram dengan `perluTinjau:false`. Sekarang `POLA_ANJURAN_MANDIRI` + `POLA_PENILAIAN` membuang kalimatnya tanpa syarat, dan pembuangan itu sendiri menandai `perluTinjau`. Tes: `tests/unit/agent/guard.test.ts`, `tests/unit/jaga/penjelasan.test.ts`.
